{
  ...
  outputs = { self, nixpkgs }:
    ...
    in
    {
      ...
      # that takes this repo as a flake input gets to install: a package and,
      # below, a home-manager module, so the input can name a ref and be bumped
      # with `nix flake update` rather than carrying a rev that has to be
      # edited by hand.
