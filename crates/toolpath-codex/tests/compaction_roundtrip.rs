...
/// Item kinds in stream order: `T` for a turn, `compacted` for the typed
/// compaction boundary, or the event's type.
fn item_shape(view: &ConversationView) -> Vec<String> {
    view.items
        .iter()
        .map(|item| match item {
            Item::Turn(_) => "T".to_string(),
            Item::Event(e) => e.event_type.clone(),
            Item::Compaction(_) => "compacted".to_string(),
        })
        .collect()
}

#[test]
fn compaction_keeps_its_stream_position_through_roundtrip() {
    let original = load_view(synthetic_fixture_path());
    let expected = [
        "session_meta",
        "task_started",
...
///
/// The real capture's `message` is empty, and an empty message normalizes
/// to `summary: None` on the way back in — the assertion below compares
/// against the original view's summary, whatever shape it took.
#[test]
fn real_fixture_projection_round_trips_compaction() {
    ...
    let follows_marker = session.lines.windows(2).any(|w| {
        w[0].kind == "compacted"
            && w[1].kind == "event_msg"
            && w[1].payload.get("type").and_then(|t| t.as_str()) == Some("context_compacted")
    });
    assert!(
        follows_marker,
        "compacted must be followed by context_compacted"
    );
