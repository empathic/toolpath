...
//!     at its position in `view.items`, parented on the preceding turn
//!     (this is the documented contract), not a generic
//!     `ConversationEvent`.
//!   - The compaction boundary survives the IR derive/extract round-trip
//!     as a `conversation.compact` step — keeping its stream position and
//!     parent — and the user/assistant content surrounding it survives
//!     too, with the projector emitting a functionally equivalent
//!     `Session`.
...
fn to_view_surfaces_compaction_as_compaction_item() {
    ...
    // The boundary rides on msg_a1's part list, so it is emitted before
    // msg_a1's turn and parents on msg_u1 (the last turn pushed when the
    // part was reached).
    assert_eq!(c.parent_id.as_deref(), Some("msg_u1"));
    ...
    // In-stream position: between msg_u1's turn and msg_a1's turn.
    let pos = view
        .items
        .iter()
...
fn compaction_item_survives_derive_extract() {
    ...
    let pos = after
        ...
        .iter()
        .position(|i| matches!(i, Item::Compaction(_)))
        .expect("compaction boundary dropped by derive → extract roundtrip");
    ...
    // Parent survives the round-trip: the boundary still parents on the
    // turn preceding it in the stream.
    let c = after.items[pos].as_compaction().unwrap();
    assert_eq!(c.parent_id.as_deref(), Some(before_turn.id.as_str()));
}
...
fn projector_emits_session_with_pre_and_post_compact_messages() {
    ...
    // both assistant responses. The compaction itself projects to a
    // synthetic compaction-bearing user message (counted among the user
    // messages here).
