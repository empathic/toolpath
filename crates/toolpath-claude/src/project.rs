...
mod tests {
    ...
    #[test]
    fn test_parallel_tool_calls_keep_the_harness_shape() {
        // Claude Code writes each tool_use of one message as its own
        // assistant line, chained, and every result after the last one,
        // each hanging off its own call line. A hook line hangs off the
        // last line written when the hook ran. File order and parents:
        //   u1, a1 <- u1, a2 <- a1, h1 <- a2, t1 <- a1, h2 <- t1,
        //   t2 <- a2, n1 <- t2, a3 <- n1
        // t1 and h2 sit after a2 while their parent chains lead to a1.
        let mut a1 = assistant_turn("a1", "");
        a1.parent_id = Some("u1".into());
        ...
        view.events = vec![
            ...
            t1,
            h2,
            tool_result_event("t2", "a2", "toolu_2"),
            attachment("n1", Some("t2"), "2024-01-01T00:00:01Z"),
        ];

        let convo = ClaudeProjector.project(&view).unwrap();

        let topology: Vec<(&str, Option<&str>)> = convo
            .entries
            .iter()
            .map(|e| (e.uuid.as_str(), e.parent_uuid.as_deref()))
            .collect();
        assert_eq!(
            topology,
            [
    ...
    fn test_tool_result_event_without_parts_or_message_is_an_error() {
        ...
        let mut bare = attachment("tr1", Some("a1"), "2024-01-01T00:00:01Z");
