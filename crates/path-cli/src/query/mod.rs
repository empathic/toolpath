...
pub fn run(
    ...
    execute_plan(config_dir, scope, &plan, code, compact, raw, &mut out)?;
...
fn execute_plan(
    ...
    match plan {
        plan::Plan::Slurp => filter::execute(plan, code, compact, raw, out, |emit| {
            stream_files(config_dir, scope, emit)
        }),
...
fn for_each_file<T: Send>(
    ...
    let sources = select_files(config_dir, scope)?;
...
/// order given.
fn select_files(config_dir: &FsPath, scope: &Scope) -> Result<Vec<DocSource>> {
    ...
    if load_cache {
        ...
        for entry in crate::cache::list_cached(config_dir)? {
...
mod tests {
    ...
    fn select_files_input_only_skips_cache() {
        ...
        let files = select_files(FsPath::new("/nonexistent"), &scope).unwrap();
