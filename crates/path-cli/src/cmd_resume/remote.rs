...
fn print_plan(plan: &RemotePlan, dest: &Destination) {
    let action = match (plan.action, plan.dead_session) {
        (RunAction::Attach, _) => "attach to the live session. The remote tree and turns are kept.",
        ...
        (RunAction::Ship, false) => "ship, launch, attach.",
        (RunAction::Ship, true) => "ship, kill the dead tmux session, launch, attach.",
    };
