...
path-cli (binary: path)
 ...
 ├── toolpath-openclaw → toolpath, toolpath-convo
...
Cross-dependencies between satellite crates: `toolpath-claude → toolpath-convo`, `toolpath-gemini → toolpath-convo`, `toolpath-codex → toolpath-convo`, `toolpath-opencode → toolpath-convo`, `toolpath-cursor → toolpath-convo`, `toolpath-pi → toolpath-convo`, `toolpath-openclaw → toolpath-convo`.
...
cargo run -p path-cli -- p import openclaw --agent main --session <session-id>   # OpenClaw agent session
...
- `toolpath-convo`: 120 unit + 4 doc tests (types, enrichment, display, ConversationView -> Path derivation, message-group usage accounting, breakdowns, typed fidelity fields: signatures/response_model/markers)
...
- `toolpath-openclaw`: 52 unit + 7 integration tests (v3 schema serde, path/session-key resolution, reader + tree/visible-leaf assembly, listing, session_to_view mapping + channel-aware actor, derive, projector inception + raw-JSON sessions.json merge, full projection round-trip, and a real-session suite over the captured fixtures in `test-fixtures/openclaw/` — telegram channel actor, spawn-child kind, real tool vocabulary, fidelity round-trip of signatures/responseId/responseModel/markers, cumulative-usage aggregate detection)
- `toolpath-dot`: 30 unit + 2 doc tests (render, visual conventions, escaping)
- `path-cli`: 300 unit + 65 integration tests (import/export/cache, track sessions, merge, validate, roundtrip, render-md snapshots, deprecation aliases, pathbase HTTP mock-server tests, fzf-friendly TSV output, openclaw import + share aggregation + inception+ resume, `path resume` orchestration with injectable `ExecStrategy`). For an end-to-end check against a real Pathbase deployment, run `scripts/test-pathbase-live.sh <url>` — it does an anon round-trip in a sandboxed config dir and, if you're logged into that URL, an authed pathstash round-trip too.
...
- OpenClaw provider: `toolpath-openclaw` reads OpenClaw agent-session JSONL from `~/.openclaw/agents/<agentId>/sessions/<sessionId>.jsonl` (override the state dir with `OPENCLAW_STATE_DIR`/`OPENCLAW_HOME`; legacy `~/.clawdbot`). Format is **version 3**: a session header line + an `id`/`parentId` entry tree whose visible head is moved by `leaf` rows (same family as Pi). Validated against real sessions captured from the official Docker image (`scripts/openclaw-docker.sh`; fixtures at `test-fixtures/openclaw/`, wired into the cross-harness matrix). Observed native tools: `exec`/`read`/`write`/`edit`/`sessions_spawn` (delegation); sub-agent runs get `agent:<id>:subagent:<uuid>` keys → sessionKind `spawn-child`, no channel actor. OpenClaw is **multi-channel** — the human isn't a transcript field; identity lives in the `sessions.json` routing key `agent:<id>:<channel>:<peerKind>:<peerId>`, which the provider recovers into a **channel-aware** `human:<channel>/<peerId>` actor (via the additive `DeriveConfig.user_actor`, kind v1.x) and stashes (channel/peer/sessionKey/sessionKind) on `path.meta.extra["openclaw"]`. **No file diffs** are stored, so changes are structural/tool-input-only (no `raw` perspective). The projector (`project.rs`) **incepts** a `Path` back onto disk — writing the transcript plus a `sessions.json` routing entry so a running OpenClaw instance can pick it up (`path p export openclaw --project`). CLI is agent-keyed (`--agent`, default `main`), not project-keyed. See `docs/agents/formats/openclaw/` for the full format reference.
