...
crates/
  ...
  toolpath-amp/                 # derive from + project to Amp (ampcode.com) threads via `amp threads export` (preview; import/list/show/share/export/resume wired; resume verified in amp 0.0.1785170481-ga5b614)
...
path-cli (binary: path)
 ...
 ├── toolpath-amp     → toolpath, toolpath-convo  (preview)
...
`toolpath-amp` is a **preview** provider (workspace member + `path-cli`
dependency) wired both directions: forward (`path p import / list / show amp`,
`path share`) and reverse via an `AmpProjector` (`path p export amp`,
`path resume`). Amp threads are **server-authoritative** — no complete local
record exists on disk — so forward derivation shells out to the first-party
`amp threads export <id>` (injectable `ThreadFetcher`; `DirFetcher` reads
pre-exported files offline), and resume creates a fresh server-side thread
with `amp threads new` and seeds it via `amp threads continue <id> -x`
(rendered-transcript **context transfer**, not native tool blocks — the
full-fidelity projection is filed at `~/.toolpath/amp-projected/<id>.json`,
INSERT-only). **✅ Verified in amp 0.0.1785170481-ga5b614:** resumed threads
answer probing questions about the prior session correctly — amp→amp and
amp→claude, plus claude→amp at 0.0.1785228716-gedda19. The writer-route
evidence (local fabrication impossible, the REST-looking import answers
`201 Created` and creates nothing, the first-party CLI two-step chosen) lives
in `docs/agents/formats/amp/writing-compatible.md`. Also validated by the
cross-harness matrix + round-trip tests.
...
cargo run -p path-cli -- p import amp --session T-<thread-id>   # via `amp threads export` (preview)
...
cargo run -p path-cli -- p import claude --project . --no-cache | path p render md
...
- `toolpath-amp`: 126 unit + 8 integration + 2 doc tests (export-document wire model with `#[serde(flatten)]` extras and whole-document **value-identity** round-trip — incl. explicit-null cache counters via double-`Option`, empty `messages`/`content` arrays, and a tripwire pinning the residual scalar-null normalization — injectable `ThreadFetcher` (`CliFetcher` shell-out / `DirFetcher` offline with distinct not-found/ambiguous/IO errors — `cargo test` never touches the network), thread-feeding `CliWriter` stdin (no deadlock on chatty children; stderr wins over EPIPE) + strict `create_thread` id parse, `to_view` turn assembly incl. tool-result-only `user` messages merged onto the originating invocation, malformed-known-block drift warnings, `Task` → `DelegatedWork` by `TU-…` id, `apply_patch` per-file `FileMutation`s with the wire's real unified diffs, clean per-message token stamping (no zero stamping; saturating totals; `totalInputTokens`/`maxInputTokens` wire-preserved but never summed), max-of-signals `last_activity` (the export's top-level `updatedAt` is frozen at creation in the real capture), kind-conformance against the bundled v1.1.0 schema, `AmpProjector` position-stable ids + token re-expansion incl. counter-free model/timestamp stubs + `native_name` remap with arg-shape synthesis (patchText envelopes, `finder {query}`, shell `cmd`→`command`), and a foreign-claude-session round-trip). Ships the real captured feature-elicit thread at `tests/fixtures/real-session.json` (byte-identical to `test-fixtures/amp/convo.json`) driving `real_fixture_roundtrip.rs`. The projector is exercised by the cross-harness matrix in `path-cli`.
...
- `toolpath-cursor`: 81 unit + 8 integration round-trip + 1 real-DB sanity + 1 doc test (state.vscdb SQLite reader, bubble store + composer header parsing, content-addressed blob lookup, projector with full TOOL_TABLE coverage incl. `*** Begin Patch` envelope path/content recovery, JSONL transcript ingest in `examples/dump_fixture.rs`)
...
- `path-cli`: 364 unit + 127 integration tests (import/export/cache, track sessions, merge, validate, roundtrip, render-md snapshots, deprecation aliases, pathbase HTTP mock-server tests, fzf-friendly TSV output, the cross-harness conformance matrix (`tests/cross_harness_matrix.rs` — every provider fixture through every projector, incl. copilot and amp), `path resume` orchestration with injectable `ExecStrategy`, `path query`/`path kind` jaq filters + kind-selector matching + step wrapping over a `$TOOLPATH_CONFIG_DIR` cache sandbox, streaming-planner recognition + streamed-output-equals-slurp equality checks, `p cache sync` incremental ingestion — stat fingerprints, refresh-overwrite, per-type filtering, failure tallying — over resolver-injected provider fixtures, and import/share manifest recording end-to-end). For an end-to-end check against a real Pathbase deployment, run `scripts/test-pathbase-live.sh <url>` — it does an anon round-trip in a sandboxed config dir and, if you're logged into that URL, an authed pathstash round-trip too.
...
- Tier 2: `toolpath-convo` (depends on `toolpath`); then `toolpath-git`, `toolpath-github`, `toolpath-dot`, `toolpath-md`, `toolpath-claude`, `toolpath-gemini`, `toolpath-codex`, `toolpath-copilot`, `toolpath-amp`, `toolpath-opencode`, `toolpath-cursor`, `toolpath-pi`
...
- Provider-specific extras: `Turn` has **no** `extra` escape hatch (removed in `0452f61`) — everything a consumer or projector needs must live in typed IR fields, and projectors synthesize provider-specific UI/decoration fields from `args` + results rather than reading stashed originals. The only provider-namespaced payload left is `WatcherEvent::Progress.data` (e.g. `data["claude"]`, `data["gemini"]`), which watchers use for live progress metadata.
...
- Amp provider (**preview, schema reverse-engineered**): `toolpath-amp` derives from Amp (ampcode.com) threads. Threads are **server-authoritative** — the per-thread log under `~/.cache/amp/logs/threads/` is content-free telemetry, and no other local file holds thread content — so the canonical artifact is the `amp threads export <id>` document, fetched by shelling out to the first-party `amp` CLI (inheriting its login) behind an injectable `ThreadFetcher` (`CliFetcher` live / `DirFetcher` for pre-exported files and tests; `cargo test` never touches the network). Threads are global (`T-<uuidv7-ish>` ids, not project-keyed); listing costs one `export` per row (N+1, accepted). The export carries full text, thinking, tool calls with parsed results, and **clean per-message token usage** — the four real counters go on `Turn.token_usage` verbatim (`group_id`/`attributed_token_usage` stay `None`, `breakdowns` omitted — no reasoning counter exists in Amp at all), while `totalInputTokens` (a derived sum, verified `= input + cacheRead + cacheCreation` on every captured usage object) and `maxInputTokens` (the 272k context-window **capacity**) are wire-preserved for round-tripping but never summed into any spend figure. Tool-result-only `user` messages are transport plumbing: results merge onto the originating invocation and no turn is emitted. `apply_patch` results carry per-file real unified diffs (`raw` perspective); errors signal via `run.result.exitCode != 0` only — `run.status` is `"done"` even on failure. Sub-agents (`Task`) return a bare string with no child thread, so `DelegatedWork.turns` stays empty (like Claude/Copilot). No git state is recorded, so `Path.base.vcs_*` is always `None`. Wired both directions: forward (`path p import/list/show amp`, `path share`) and reverse via `AmpProjector` (`path p export amp`, `path resume`): export `--output`/stdout emits the projected thread document offline; `--project`/resume **create a fresh server-side thread** (`amp threads new`, server-assigned id, never touching existing threads) and seed it with a rendered transcript (`amp threads continue <id> -x`, prompt on stdin to dodge `ARG_MAX`) — **context transfer, not native tool blocks** (Amp accepts no document import; the REST-looking `POST /import` answers `201 Created` and creates nothing — writer evidence in `docs/agents/formats/amp/writing-compatible.md`). The full-fidelity projection is filed INSERT-only at `~/.toolpath/amp-projected/<thread-id>.json` under the live thread id. **✅ Verified in amp 0.0.1785170481-ga5b614** (probing-question passes amp→amp and amp→claude; claude→amp at 0.0.1785228716-gedda19); `scripts/verify-amp-live.sh` reruns the loop in an isolated home. Amp versions are build-timestamped and churn fast — every format claim is version-stamped; trust the per-thread `env.initial.platform.clientVersion` anchor. Full format reference: `docs/agents/formats/amp/`.
