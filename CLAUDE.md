...
# Continuous sync: one pass over the sessions in [sync] scope (needs login)
cargo run -p path-cli -- sync --dry-run
cargo run -p path-cli -- sync install --include ~/work --interval 15m   # writes [sync], schedules `path sync`
cargo run -p path-cli -- sync status
cargo run -p path-cli -- sync uninstall
...
- `path share` is the one-shot `p import <harness> | p export pathbase`: probes installed harnesses, aggregates sessions into one picker ranking current-directory sessions first; `--harness`/`--session`/`--project` skip the picker; pathbase flags match `p export pathbase`. When the sync manifest shows the picked session unchanged (`sync::fresh_cache_id`), it uploads the cached doc instead of re-deriving. Uploads carry the same full derivation as local projection — no egress stripping. **Authed share runs through the sync engine** (`sync::pass::sync_session` with `freeze: false`): it takes the upload lock (`sync::lock::lock_uploads`), probes the server for the sync API (`sync::lock::supports_sync`; a server without it is an upgrade error, never a duplicate-producing create), then creates a graph when the session is untracked, `PUT`s the mutable graph when the session changed, continues a frozen graph when there are new steps, and skips an unchanged one (`Already uploaded to <repo>, unchanged since; pass --force to upload again`, printing the recorded URL). It writes the same records as `path sync`: `SyncRecord.uploads` on the manifest (graph id, URL, source stamp; the URL `<server>/u/<owner>/<name>/graphs/<id>` is what destination matching keys on) and the `sync-state/` sidecar, so manual and automatic uploads agree on state. Share never freezes; that is `path sync`'s policy. `share --all` skips the picker and uploads every new or changed session in scope (`--project-under` subtree, `--harness`), skipping unchanged ones, printing a per-project summary (`N to update`, `N to continue`, `N already uploaded`) and confirming first; `--dry-run` runs the pass's dry run per session; it needs login. `--force` is the design's manual force: it re-sends an unchanged mutable graph as a `PUT` the server treats as a no-op and replaces the graph's owned content even when the source lost acknowledged steps (`PassContext.force`); it never reopens a frozen graph (a frozen graph with new steps still continues, with none says `already frozen`). Anonymous share (`--anon`, or logged out) stays outside the engine: `cmd_export::run_pathbase_inner` creates a frozen anonymous graph and records nothing. `p export pathbase` is the one remaining direct authed create — for a document that is not a live session, untracked, mutable — and it refuses a server without the sync API the same way. `put_record` in `sync/engine.rs` carries `uploads` across sync/import rewrites.
...
### Continuous sync (`path sync`)

- Design: `docs/superpowers/specs/2026-09-02-continuous-sync-design.md`; wire contract with Pathbase: `docs/superpowers/specs/2026-09-10-continuous-sync-api-contract.md`. A graph is mutable while a session is active; after **two hours of source inactivity** the next successful pass freezes it (one-way); resumed work goes into a continuation graph that owns only new steps and references the frozen head through `path.base.from` (`<graph-url>#<path-id>/<step-id>`, `toolpath::v1::BaseReference`).
- Modules under `crates/path-cli/src/sync/`: `activity` (inactivity clock from source stamps; provider timestamps trusted only in the past, changes advance to observation time, unknown freshness never idles), `segment` (live derive → what the current graph may own; frozen ids dropped, new steps must hang off the frozen head, frozen parents trimmed, root attaches via `base.from`; anything else is an unsupported shape), `journal` (staged operation = exact bytes + `Idempotency-Key` + precondition under `pending/`, 0600), `state` (per-session `sync-state/<harness>-<session>.json`: current graph's acknowledged owned ids and the frozen boundary, whose id set accumulates along the chain), `api` (`meta`, `execute(op)`, `stored_document` over the generated `pathbase-client` — `get_graph_meta`, `create_graph`, `replace_graph`, `freeze_graph`, `create_continuation`, `download_graph`, with the `Idempotency-Key` as the generated header parameter; staged bytes are the serialization of the generated body type, parsed back at send time so a replay sends identical bytes; failures classified by the typed `ApiErrorCode` first and the status only as a fallback, transport errors and 5xx are `Ambiguous` and keep the operation staged, a response that does not fit the generated types is `UpgradeRequired`; request bodies are not gzipped), `lock` (the upload lock and the `supports_sync` probe every uploader runs first), `pass` (the per-session decision table, tested against an in-memory fake server; `PassContext` carries `force`, `freeze`, and the create's `name`/`visibility`, so `share` and `sync` differ only in those flags). `cmd_sync.rs` is the command: config → scope → login → upload lock → `sync_bundle` ingestion → per-session pass → `sync-status.json`; nonzero exit when anything failed or stayed pending. `install` preflights before writing anything: credentials against every configured server (`api_me`) and every configured repo (`repo_get`); the server comes from a URL-form remote, else `$PATHBASE_URL`, else the default; `--url` is a one-run override and is never persisted. The user's `pathstash` is seeded by the server at sign-in, so sync does not create repos.
- Config: `sync_config.rs` (`[sync]`, `ScopeOverrides`, `EffectiveScope`, destination precedence `--repo` > project remote > `[sync].remote` > `<you>/pathstash`; `sync = false` is never bypassed; `enabled = false` is authoritative), `sync_service.rs` (config merge for install/uninstall, launchd/systemd templates). `share_config::resolve_project_fields` resolves `remote` and `sync` independently; manual share keeps first-matching-origin precedence, sync refines an origin match by dir depth.
- Rules the code enforces: never send without staging first; never record an upload without a server acknowledgement; replay an ambiguous operation before creating another; a definitively rejected PUT after a freeze is retired and re-segmented as a continuation; an existing main-line continuation is adopted only when what it owns is a prefix of what we would send; remote drift that is not a display edit is a conflict, never an overwrite; a deleted graph is reported, never recreated by sync; a source regression (acknowledged owned steps missing from the derive, `Segmentation::Document.missing`) is reported unless `force` is set, which only a manual `share --force` does.
...
- Manifest at `~/.toolpath/manifest.json`: artifact type → id → `{path?, cache_id, modified?, size?, synced_at, uploads?}`. Atomic temp+rename writes, advisory lock with read-merge-save, checkpointed every 10 writes — concurrent invocations union their records, and an interrupted run keeps what it derived (derives run newest-first).
