...
- `/path:resume` projects a shared session into the current project via `p import pathbase` + `p export claude` and hands the user `/resume <id>` — the running TUI cannot be switched programmatically, and the command guards against re-exporting a session that already exists locally (export overwrites the file). With `--remote <user@host>` a `UserPromptExpansion` hook with matcher `path:resume` (`hooks/hooks.json` → `scripts/resume-remote-hook.sh`) runs when the command expands. The hook classifies, the CLI parses: it takes the binary from `ensure-path.sh which` (resolve only, never install, since a hook runs with no permission prompt), and when the arguments carry `--remote` and no document, it forwards them verbatim to `resume --session <id> --project <cwd> --no-attach` (no `--no-attach` with `--dry-run`) and blocks the expansion with the `ssh -t` attach line, or the CLI's error, as the reason, so the send costs no model turn. The cases where it lets the expansion reach the command are listed in the command's Remote section (`commands/resume.md`), which then sends the document or tells the user what to install. `/path:link-pr` runs the share flow and appends the link to a PR description via `gh pr view/edit`.
