...
crates/
  ...
  toolpath-copilot/             # derive from + project to GitHub Copilot CLI session logs (preview; import/list/show/share/export/resume wired; real-CLI resume load unverified)
...
**Caveat:** whether the real `copilot --resume` loads a synthesized session is
**unverified** (can't be tested without the CLI); the projector is validated in
