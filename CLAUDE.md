...
follow a branch of this repo instead of pinning a rev by hand (`bdelanghe/home` does this
for lobby; `bdelanghe/empathic-nix` still carries its own pinned build of path-cli):
...
The package skips the workspace tests (`doCheck = false`); CI is the gate. CI runs the `ci`
job on pushes to `bdelanghe/wip` as well as `main`, so a follower of wip only ever moves to
a green rev; the `deploy` job stays gated to main and pull requests.
