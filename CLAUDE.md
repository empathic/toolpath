...
- `ConversationMetadata`/`SessionMeta` for claude/gemini/pi expose `first_user_message: Option<String>` (cheap, populated during the metadata pass) — used by pickers and any "list sessions by topic" surface. Claude's metadata also carries the session's `custom_title` (`/rename`) and `ai_title`; `ConversationMetadata::title()` picks the rename, then the generated title, then the first prompt, and the Claude pickers show it. A session chain's metadata merges through `ConversationMetadata::merge_newer_segment`, which names a rule for every field.
