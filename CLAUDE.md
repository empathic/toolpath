...
- Codex tool `output` is a string in older rollouts and an array of content parts (`input_text`/`input_image`) in newer ones; `types::output_text` accepts both.
- Claude Code reads the newest assistant line's `message.usage` as the live context size; a projection carrying another harness's counts resumes with no history. `ClaudeProjector` writes usage only when `view.provider_id` is `claude-code` (or unset); other harnesses' counts go under the entry's `toolpathUsage` key (`types::TOOLPATH_USAGE_KEY`), which the reader falls back to, so the cross-harness matrix still round-trips accounting. See hard rule 4 in `docs/agents/formats/claude-code/writing-compatible-jsonl.md`.
