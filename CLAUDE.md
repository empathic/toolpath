...
- `path-cli` has `embedded-picker` (default: on; the skim picker) and `resume-remote` (default: off) gating the `p export claude` flags `--content-addressed-session-id` and `--cwd`, `path resume --remote` and `--dry-run`, and the code behind them (`crates/path-cli/src/claude_session.rs`, `crates/path-cli/src/cmd_export/remote_session.rs`, `crates/path-cli/src/ssh.rs`, `crates/path-cli/src/cmd_resume/remote.rs` and its `.sh` probe script); the `russh` and `shlex` dependencies are optional on it; `scripts/resume-remote.sh` builds with it. The gate is `all(feature = "resume-remote", not(target_os = "emscripten"))`: the feature has no effect on the wasm build. Test both states: `cargo test -p path-cli` and `cargo test -p path-cli --features resume-remote`.
