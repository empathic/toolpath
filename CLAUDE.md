...
- Config: `sync_config.rs` (`[sync]`, `ScopeOverrides`, `EffectiveScope`, destination precedence `--repo` > project remote > `[sync].default_remote` > `<you>/pathstash`; `sync = false` is never bypassed; `enabled = false` is authoritative), `sync_service.rs` (config merge for install/uninstall, launchd/systemd templates). `share_config::resolve_project_fields` resolves `remote` and `sync` independently; manual share keeps first-matching-origin precedence, sync refines an origin match by dir depth.
