...
- `toolpath-openclaw`: 48 unit + 1 integration test (v3 schema serde, path/session-key resolution, reader + tree/visible-leaf assembly, listing, session_to_view mapping + channel-aware actor, derive, projector inception + sessions.json, full projection round-trip)
