...
- `toolpath-copilot`: 49 unit + 4 integration + 1 doc test (tolerant `events.jsonl` envelope/event-type classification, path resolution incl. legacy `history-session-state/`, reader malformed-line tolerance, tolerant `workspace.yaml` git-context parse, `to_view` turn/tool/delegation assembly, id-based **and** id-less positional tool start/complete pairing, file-write `raw` diff synthesis, derive against a **synthetic** fixture). Preview crate: fixtures are synthetic (no first-hand Copilot sessions); replace with a real capture once available — see `docs/agents/formats/copilot-cli/known-gaps-and-sourcing.md`.
