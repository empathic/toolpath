...
- Message-level token accounting (kind v1.1.0): `token_usage` always means "the total for a message"; one provider message can span several steps (Claude Code writes one JSONL line per content block). `Turn.message_id` groups them, the total sits on the group's final turn/step (the IR is canonical — provider `to_view`s never emit repeated or zero-filled usage; projectors re-expand wire quirks like Claude's per-line duplication), and per-step sums equal session totals. `toolpath-claude` fills `message_id` from the JSONL `message.id`; `toolpath-codex` groups a round's turns under its `turn_id` and accumulates `token_count` spend onto the round's final assistant turn (preferring `last_token_usage` over cumulative-total deltas); pi/opencode decode all-zero wire counters as `None`. Never stamp a cumulative counter, a repeated message total, or zero-filled placeholders onto a step.
