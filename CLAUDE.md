...
Object names are `<date>-<topic>--<id>.json` (`store::name_for`), where the ID is the session's own conversation artifact key (`store::session_key`: the key of the `conversation.append` entry, which the agent-coding-session kind specifies as `<source>://<session-id>`, slugified) and `--` is reserved (the slugger collapses dash runs, so neither date nor topic can contain it; `ObjectName::id_of` splits on the last `--`, and a name with no date or topic is `<id>.json`). Every component is a pure function of the document — never of the input filename — so a re-export overwrites its own object, two different documents with the same basename land on two keys, and the date is the *earliest* step's so it doesn't move as the session grows. Keying on the session means two objects collide only if a harness issued one session ID twice — nothing is truncated or hashed to fit. A document with no conversation artifact (git-derived, hand-written) falls back to `graph.id`, which is not unique across a shared bucket: two git documents from different repositories on the same branch both derive `path-main`, and `--no-overwrite` is the guard. Imports cache under `object-<id>` (`ObjectUri::cache_id`, from the URI alone), so a round trip is identity-preserving and a mirror loop is a fixed point. `p export object` validates the body (parse + schema) before any put; `--force` uploads anyway. Every upload is recorded in `~/.toolpath/exports.json` (`export_ledger`: destination → cache ID → uri, sha256, bytes, time, uploader), which `p export object --all` uses to skip unchanged documents and which doubles as the local egress record. Overwrite is the default put; `--no-overwrite` (or `no_overwrite` in `s3.json`) is a create-only put. S3 objects carry `x-amz-meta-toolpath-*` metadata (graph ID, sha256, uploader, CLI version, time); folders can't, and get 0600 files / 0700 created directories instead.
...
- All configuration flows through figment: `crates/path-cli/src/config.rs::Config::load` is the composition root — defaults, then `~/.toolpath/config.toml` (`Toml::file`, so `[share]` fills `Config::share` and the `[[project]]` rules are ignored there), then the mapped environment variables on top. `Config::load_env` is the environment-only phase that locates the file; the transitional free `config_dir()` and `path config edit` run on it so a file that fails to parse can still be opened and fixed. New settings get a field on `Config` and a layer in `load`, never an ad hoc `std::env` or `toml::from_str` read at the use site.
