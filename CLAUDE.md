...
`toolCallId`, full `session.start` shape) — see
`docs/agents/formats/copilot-cli/writing-compatible.md`. Verified on a real
27-event session; the same requirements + defensive handling (e.g. synthesized
`toolCallId` when empty) cover larger docs, and the projector is also validated
by the cross-harness matrix + a round-trip test.
