...
- `path-cli` has `embedded-picker` (default: on; the skim picker) and `resume-remote` (default: off) gating the `p export claude` flags `--content-addressed-session-id` and `--cwd`, `path resume --remote`, `--dry-run`, and `--no-attach`, and the code behind them (`crates/path-cli/src/claude_session.rs`, `crates/path-cli/src/cmd_export/remote_session.rs`, `crates/path-cli/src/ssh.rs`, `crates/path-cli/src/cmd_resume/remote.rs` and its `.sh` scripts); the `russh`, `shlex`, and `crossterm` dependencies are optional on it; `scripts/resume-remote.sh` builds with it. The gate is `all(feature = "resume-remote", not(target_os = "emscripten"))`: the feature has no effect on the wasm build. Test both states: `cargo test -p path-cli` and `cargo test -p path-cli --features resume-remote`.
...
- Streamed upload: an authed upload whose serialized document is over `BATCH_BUDGET` (4 MiB, `cmd_pathbase.rs`) goes through `graphs_post_streamed` instead of the single `graphs_post`: create the graph with `paths: []`, then per inline path pack `Path::to_jsonl_writer` lines into batches of at most the budget (`pack_batches`, a pure function) — batch 1 to `POST …/graphs/{id}/paths` (response carries the server `path_id`), the rest to `POST …/graphs/{id}/paths/{path_id}/steps`, both `application/x-ndjson`. Non-final batches end with an added `Head` line naming their last step; a step's `Signature` lines stay with it; `PathOpen`/`ActorDef` lines stay with the first step; an oversized step goes alone; steps are reordered parents-first only when a child precedes its parent. Transport errors and 5xx retry up to 3 times with backoff; any other failure, or Ctrl-C (a `tokio::signal::ctrl_c` listener on the shared runtime, raced against each batch request), deletes the graph (best effort) and returns the error. Anonymous uploads, documents at or under the budget, and documents with a `$ref` entry or a step-less path stay on the single request. A `404`/`405` on a path's first batch means the server lacks the batch routes: delete the empty graph, print a note, and fall back to the single `graphs_post` with the full document (no fallback on any other status or on later batches). The batch routes are the generated `open_graph_path`/`append_graph_path_steps`, called from a second typed client built with a 120 s timeout; `crates/pathbase-client/build.rs` maps their `application/x-ndjson` body to `text/plain` at generation time (the committed spec keeps `x-ndjson`).
