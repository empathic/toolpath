...
- `toolpath-convo`: 115 unit + 7 doc tests (types, enrichment, display, ConversationView -> Path derivation, message-group usage accounting)
...
- `toolpath-claude`: 222 unit + 18 integration + 11 doc tests (path resolution, conversation reading, query, chaining, watcher, derive, metadata first-user-message, message_id grouping + once-per-message usage totals)
...
- `toolpath-codex`: 76 unit + 50 integration + 2 doc tests (rollout parsing, provider assembly, patch-fidelity derive, real-session fixture, source→path fidelity invariants, JSON wire-level round-trip, per-turn token deltas from cumulative counters)
...
- Token accounting (kind v1.1.0): two keys on `conversation.append`/`Turn`, both optional. `token_usage` = "the total for a message" (on the group's final step; `Σ` over a path = session total). `attributed_token_usage` = "this step's own attributed spend" (its own key, so the sum is unaffected; remainder = group total − Σ attributed, computed not stored). One provider message can span several steps (Claude writes one JSONL line per content block); `Turn.message_id` groups them. The IR is canonical — provider `to_view`s never emit repeated or zero-filled usage; projectors re-expand wire quirks (Claude's per-line cumulative; Codex's per-step `token_count`). `toolpath-claude` fills `message_id` from `message.id`, takes the field-wise-max group total (line order not trusted — ~27% of splits disagree), and attributes streamed `output_tokens` deltas. `toolpath-codex` differences the cumulative `total_token_usage` (dedup-safe: each `token_count` is emitted twice — never sum `last_token_usage`), attributes each delta to the step it follows, and derives the round total from those attributions. pi/opencode decode all-zero wire counters as `None`. Never stamp a cumulative counter, a repeated message total, or zero-filled placeholders onto a step.
